...
def precompute_filter_options(scene_struct, metadata, remove_redundant=False):
    ...
    if args.contain_pose:
        ...
        attr_keys = ['pose', 'size', 'color', 'shape']
    else:
    ...
    while states:
        ...
        if state['next_template_node'] == len(template['nodes']):
            ...
            if cur_answer_count > 1.1 * answer_counts_sorted[-2]:
                if verbose: print('skipping due to second count', answer_counts_sorted, answer, answer_counts)
                reject_count += 1
                continue
...
def main(args):
    ...
    if "_aux" in args.output_questions_file and os.path.exists(args.output_questions_file.replace("_aux", "")):
        with open(args.output_questions_file.replace("_aux", ""), 'r') as f:
            data = json.load(f)
